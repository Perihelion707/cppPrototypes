...
sf::Vector2f tryBounce(
    sf::Vector2u screenSize, sf::Vector2f rectangleSize,
    sf::Vector2f rectanglePosition,
    sf::Vector2i localPosition
  ) {
  sf::Vector2f tempVelocity = velocity;

  int xBoundary = screenSize.x;
  int yBoundary = screenSize.y;
  int xSizeBuffer = rectangleSize.x;
  int ySizeBuffer = rectangleSize.y;

  if (rectanglePosition.x > screenSize.x ||
...
int main() {
  sf::RenderWindow window(sf::VideoMode({1200, 800}), "DVD-Screen", sf::Style::None,
                          sf::State::Windowed);
